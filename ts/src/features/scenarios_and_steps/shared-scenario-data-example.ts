...
export async function build(): Promise<unknown> {
  ...
  const feature = { scenario: scenario("shared-data"), sharedData: {} };
  const result = await LoadStrikeRunner.registerScenarios(scenario("shared-scenario-data")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("shared-scenario-data").withoutReports().run();
