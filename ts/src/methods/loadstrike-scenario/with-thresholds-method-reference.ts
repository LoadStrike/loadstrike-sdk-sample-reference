...
export class WithThresholdsMethodReference {
  /** Attach one scenario-level threshold. */
  static async AttachScenarioThresholdExample(): Promise<unknown> {
      return baselineScenario().withThresholds(LoadStrikeThreshold.CreateScenario(context => context.runStats.allRequestCount >= 1, 1, 2));
  }

  /** Attach more than one threshold in the same call. */
  static async AttachScenarioAndStepThresholdExample(): Promise<unknown> {
      return baselineScenario().withThresholds(LoadStrikeThreshold.CreateScenario(context => context.runStats.allRequestCount >= 1, 1, 2), LoadStrikeThreshold.CreateStep("get-order", context => context.runStats.allOkCount >= 1, 1, 2));
  }
}
