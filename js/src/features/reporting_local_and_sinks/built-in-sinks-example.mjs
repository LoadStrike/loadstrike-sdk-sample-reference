...
export async function build() {
  ...
  const featureBundle = builtInSinks();
  const result = await LoadStrikeRunner.registerScenarios(scenario("built-in-sinks")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("built-in-sinks").withoutReports().run();
