using System;
    ...
    /// <summary>Set the minimum logging level.</summary>
    public static class WithMinimumLogLevelMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Lower the logger to warning-level output.</summary>
public static object? SetWarningLevelExample()
{
    return LoadStrikeRunner.WithMinimumLogLevel(baseContext(), LogEventLevel.Warning);
}

/// <summary>Raise the logger detail for troubleshooting a sample run.</summary>
public static object? SetDebugLevelExample()
{
    return LoadStrikeRunner.WithMinimumLogLevel(baseContext(), LogEventLevel.Debug);
}
    }
