...
FEATURE_OVERVIEW = "This feature area focuses on HTTP source, destination, response tracking, and OAuth2 client-credentials configuration."


def build():
    # Http Source And Destination keeps the scenario setup focused on one behavior. Change the option below first when comparing suboptions.
    scenario = {"source": _http_source(), "destination": _http_destination(), "scenario": _tracked_scenario()}
    return (LoadStrikeRunner.register_scenarios(scenario).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("http-source-and-destination").without_reports().run())
