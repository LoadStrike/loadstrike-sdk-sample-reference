using System;
    ...
    /// <summary>Execute the configured workload immediately.</summary>
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Execute the baseline GET scenario directly from the fluent runner.</summary>
public static object? RunConfiguredRunnerExample()
{
    return baseRunner().Run();
}

/// <summary>Execute the same scenario from a reusable built context where that helper exists.</summary>
public static object? RunBuiltContextExample()
{
    return LoadStrikeRunner.Run(baseContext());
}
    }
