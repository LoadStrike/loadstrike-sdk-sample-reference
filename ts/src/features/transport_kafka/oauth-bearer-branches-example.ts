...
export async function build(): Promise<unknown> {
  ...
  const endpoint = new KafkaEndpointDefinition({ Name: "orders-kafka-oauth", Mode: "Produce", TrackingField: TrackingFieldSelector.parse("header:x-id"), BootstrapServers: KAFKA_BOOTSTRAP_SERVERS, Topic: ORDER_TOPIC, SecurityProtocol: "SaslSsl", Sasl: { Mechanism: "OAuthBearer", OAuthBearerTokenEndpointUrl: `${ORDERS_API_BASE_URL}/oauth/token` } as KafkaSaslOptions });
