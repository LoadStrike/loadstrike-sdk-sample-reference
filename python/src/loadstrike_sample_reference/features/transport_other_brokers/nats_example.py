...
def build():
    ...
    endpoint = _other_brokers()["nats"]
    result = (LoadStrikeRunner.register_scenarios(_scenario("nats")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("nats").without_reports().run())
