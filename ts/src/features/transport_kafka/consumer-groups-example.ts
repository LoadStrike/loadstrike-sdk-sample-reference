...
export async function build(): Promise<unknown> {
  ...
  const endpoints = { producer: kafkaPair().producer, consumer: kafkaPair().consumer, consumerGroupId: "orders-sample-group" };
  const result = await LoadStrikeRunner.registerScenarios(scenario("consumer-groups")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("consumer-groups").withoutReports().run();
