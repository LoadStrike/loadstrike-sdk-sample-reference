...
class CreateAsyncMethodReference:
    @staticmethod
    def create_async_scenario_example() -> Any:
        """Create the GET scenario from an asynchronous callback."""
        return LoadStrikeScenario.create_async("orders.get-by-id", execute_order_get)

    @staticmethod
    def create_async_scenario_with_simulation_example() -> Any:
        """Create the async scenario and then attach a simple simulation."""
        return LoadStrikeScenario.create_async("orders.get-by-id", execute_order_get).with_load_simulations(LoadStrikeSimulation.iterations_for_constant(1, 3))
