using System;
    ...
    namespace LoadStrike.SampleReference.Methods.OwnerLoadStrikeResponse;

    /// <summary>Return a successful typed payload reply.</summary>
    public static class OkWithPayloadMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Create the primary reply shape for this helper.</summary>
public static object? CreatePrimaryReplyExample()
{
    return LoadStrikeResponse.Ok(payload: new Dictionary<string, object>(StringComparer.Ordinal) { ["orderId"] = "ORD-10001" }, statusCode: "202", sizeBytes: 128, message: "accepted", customLatencyMs: 3.2);
}

/// <summary>Create an alternate reply shape that changes status or message metadata.</summary>
public static object? CreateAlternateReplyExample()
{
    return LoadStrikeResponse.Ok(payload: new Dictionary<string, object>(StringComparer.Ordinal) { ["orderId"] = "ORD-10002" }, statusCode: "201", sizeBytes: 64, message: "created", customLatencyMs: 4.6);
}
    }
