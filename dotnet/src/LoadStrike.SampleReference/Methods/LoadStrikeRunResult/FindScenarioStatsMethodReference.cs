using System;
    ...
    namespace LoadStrike.SampleReference.Methods.OwnerLoadStrikeRunResult;

    /// <summary>Look up scenario stats without throwing on missing names.</summary>
    public static class FindScenarioStatsMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Read data back from the run result surface with this helper.</summary>
public static object? ReadRunResultsExample()
{
    return runResult().FindScenarioStats("orders.get-by-id");
}

/// <summary>Continue into the scenario or step stats surface after the first lookup.</summary>
public static object? ReadDeeperResultSurfaceExample()
{
    return runResult().GetScenarioStats("orders.get-by-id");
}
    }
