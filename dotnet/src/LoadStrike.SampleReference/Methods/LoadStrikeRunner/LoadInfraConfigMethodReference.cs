using System;
    ...
    /// <summary>Load infrastructure settings from a JSON config file.</summary>
    public static class LoadInfraConfigMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Point the runner at a temporary JSON file generated inside the sample.</summary>
        public static object? UseGeneratedConfigFileExample()
        {
            var paths = writeTempConfigFiles();
return LoadStrikeRunner.LoadInfraConfig(baseContext(), paths.InfraPath);
        }

/// <summary>Load the file and continue into a built context.</summary>
        public static object? BuildContextFromGeneratedConfigExample()
        {
            var paths = writeTempConfigFiles();
return LoadStrikeRunner.LoadInfraConfig(baseContext(), paths.InfraPath);
        }
