...
export class WithRestartIterationMaxAttemptsMethodReference {
  /** Apply the option with a smaller numeric value. */
  static async ApplySmallerCountExample(): Promise<unknown> {
      return baseRunner().withRestartIterationMaxAttempts(2);
  }

  /** Apply the option with a larger numeric value. */
  static async ApplyLargerCountExample(): Promise<unknown> {
