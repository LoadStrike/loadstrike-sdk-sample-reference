...
def build():
    ...
    endpoints = _kafka_pair()[0]
    result = (LoadStrikeRunner.register_scenarios(_scenario("security-and-sasl")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("security-and-sasl").without_reports().run())
