...
FEATURE_OVERVIEW = "This feature area keeps the scenario simple so the file can focus on local-dev cluster setup and coordinator-agent wiring."
...
def build():
    ...
    context = {"cluster_id": "orders-cluster", "context": _runner().build_context().with_nats_server_url(placeholders.NATS_SERVER_URL).with_cluster_id("orders-cluster").with_agent_group("orders-agents").with_agents_count(3)}
