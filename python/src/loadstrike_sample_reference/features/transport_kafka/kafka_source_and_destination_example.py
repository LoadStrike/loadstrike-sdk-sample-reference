...
def build():
    ...
    endpoints = {"producer": _kafka_pair()[0], "consumer": _kafka_pair()[1]}
    result = (LoadStrikeRunner.register_scenarios(_scenario("kafka-source-and-destination")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("kafka-source-and-destination").without_reports().run())
