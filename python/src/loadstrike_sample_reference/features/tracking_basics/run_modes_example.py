...
def build():
    ...
    runModes = {"generate_and_correlate": "GenerateAndCorrelate", "correlate_existing_traffic": "CorrelateExistingTraffic"}
    result = (LoadStrikeRunner.register_scenarios(_scenario("run-modes")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("run-modes").without_reports().run())
    return {"runModes": runModes, "Result": result}
