...
class CreateGaugeMethodReference:
    @staticmethod
    def create_metric_example() -> Any:
        """Create the metric directly from the public metric helper."""
        return LoadStrikeMetric.gauge("orders_latency", "ms")

    @staticmethod
    def register_metric_during_init_example() -> Any:
        """Register the metric during scenario init so the GET scenario can report it."""
        metric = LoadStrikeMetric.gauge("orders_latency", "ms")
        scenario = LoadStrikeScenario.create("orders.metric", execute_order_get).with_init(lambda context: context.register_metric(metric))
        return scenario
