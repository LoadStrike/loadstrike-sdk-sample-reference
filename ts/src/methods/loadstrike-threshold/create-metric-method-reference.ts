...
export class CreateMetricMethodReference {
  /** Create the threshold from the public threshold helper. */
  static async CreateThresholdExample(): Promise<unknown> {
      return LoadStrikeThreshold.CreateMetric(context => (context.metrics.orders_total ?? 0) >= 1, 1, 2);
  }

  /** Attach the threshold to the baseline GET scenario. */
  static async AttachThresholdToScenarioExample(): Promise<unknown> {
      return baselineScenario().withThresholds(LoadStrikeThreshold.CreateMetric(context => (context.metrics.orders_total ?? 0) >= 1, 1, 2));
  }
}
