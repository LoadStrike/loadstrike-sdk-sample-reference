...
export async function build(): Promise<unknown> {
  ...
  const scenario = LoadStrikeScenario.createAsync(scenarioName("async-alias"), async (context) => publishStep(context));
  return await LoadStrikeRunner.registerScenarios(scenario).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("async-convenience-aliases").withoutReports().run();
}
