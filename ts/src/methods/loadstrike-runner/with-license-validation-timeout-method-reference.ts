...
export class WithLicenseValidationTimeoutMethodReference {
  /** Apply a shorter timeout to the runner. */
  static async ApplyShortTimeoutExample(): Promise<unknown> {
      return LoadStrikeRunner.WithLicenseValidationTimeout(baseContext(), 5);
  }

  /** Apply a longer timeout to the runner. */
  static async ApplyLongTimeoutExample(): Promise<unknown> {
      return LoadStrikeRunner.WithLicenseValidationTimeout(baseContext(), 15);
  }
}
