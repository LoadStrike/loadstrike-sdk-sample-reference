...
export class WithMinimumLogLevelMethodReference {
  /** Lower the logger to warning-level output. */
  static async SetWarningLevelExample(): Promise<unknown> {
      return LoadStrikeRunner.WithMinimumLogLevel(baseContext(), "Warning");
  }

  /** Raise the logger detail for troubleshooting a sample run. */
  static async SetDebugLevelExample(): Promise<unknown> {
      return LoadStrikeRunner.WithMinimumLogLevel(baseContext(), "Debug");
  }
}
