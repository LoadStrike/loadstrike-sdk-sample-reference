...
export async function build(): Promise<unknown> {
  // Inject Random keeps the scenario setup focused on one behavior. Change the option below first when comparing suboptions.
  const scenario = scenarioWithSimulation("inject-random", LoadStrikeSimulation.injectRandom(1, 8, 0.25, 20));
  return await LoadStrikeRunner.registerScenarios(scenario).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("inject-random").withoutReports().run();
}
...
export function scenarioWithSimulation(nameSuffix: string, simulation: Record<string, unknown>) {
