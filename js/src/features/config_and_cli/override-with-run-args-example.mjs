...
export async function build() {
  // Override With Run Args is easiest to understand when the runner stays fixed and only the external args change.
  const options = { runner: runner().loadConfig("./config/loadstrike.config.json"), args: ["--nodeType=Coordinator", "--agentsCount=2"] };
  return await options.runner.run(options.args);
}
