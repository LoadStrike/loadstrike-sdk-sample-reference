using System;
    ...
    /// <summary>Toggle console metrics output for a run.</summary>
    public static class DisplayConsoleMetricsMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Apply the boolean option in its enabled state.</summary>
public static object? ToggleOnExample()
{
    return LoadStrikeRunner.DisplayConsoleMetrics(baseContext(), true);
}

/// <summary>Apply the boolean option in its disabled state.</summary>
public static object? ToggleOffExample()
{
    return LoadStrikeRunner.DisplayConsoleMetrics(baseContext(), false);
}
    }
