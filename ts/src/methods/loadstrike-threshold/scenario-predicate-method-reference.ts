...
export class ScenarioPredicateMethodReference {
  /** Create the threshold from the public threshold helper. */
  static async CreateThresholdExample(): Promise<unknown> {
      return LoadStrikeThreshold.scenarioPredicate("scenario-ok", context => context.runStats.allOkCount >= 1, { abortWhenErrorCount: 1, startCheckAfterSeconds: 2 });
  }

  /** Attach the threshold to the baseline GET scenario. */
  static async AttachThresholdToScenarioExample(): Promise<unknown> {
      return baselineScenario().withThresholds(LoadStrikeThreshold.scenarioPredicate("scenario-ok", context => context.runStats.allOkCount >= 1, { abortWhenErrorCount: 1, startCheckAfterSeconds: 2 }));
  }
}
