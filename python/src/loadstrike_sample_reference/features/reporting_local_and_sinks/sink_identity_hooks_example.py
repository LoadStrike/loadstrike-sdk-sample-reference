...
def build():
    ...
    sink = OrdersReportingSink()
    result = (LoadStrikeRunner.register_scenarios(_scenario("sink-identity-hooks")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("sink-identity-hooks").without_reports().run())
