...
class RegisterScenariosMethodReference:
    @staticmethod
    def register_single_scenario_example() -> Any:
        """Register one baseline GET scenario in a single call."""
        return LoadStrikeRunner.register_scenarios(baseline_scenario())

    @staticmethod
    def register_multiple_scenarios_example() -> Any:
        """Register multiple scenarios without switching back to the builder flow."""
        return LoadStrikeRunner.register_scenarios(baseline_scenario(), baseline_scenario("orders.audit"))
