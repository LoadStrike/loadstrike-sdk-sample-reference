...
def build():
    ...
    source = _http_source()
    result = (LoadStrikeRunner.register_scenarios(_scenario("oauth2-client-credentials-auth")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("oauth2-client-credentials-auth").without_reports().run())
