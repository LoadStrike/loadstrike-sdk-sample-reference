...
export async function build(): Promise<unknown> {
  ...
  const runModes = { generateAndCorrelate: "GenerateAndCorrelate", correlateExistingTraffic: "CorrelateExistingTraffic" };
  const result = await LoadStrikeRunner.registerScenarios(scenario("run-modes")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("run-modes").withoutReports().run();
  return { runModes, result };
}
