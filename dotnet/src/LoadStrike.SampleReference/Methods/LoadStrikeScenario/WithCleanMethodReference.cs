using System;
    ...
    /// <summary>Attach a synchronous cleanup hook.</summary>
    public static class WithCleanMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Attach a no-op lifecycle hook for the baseline scenario.</summary>
public static object? AttachNoOpHookExample()
{
    return baselineScenario().WithClean(context => Task.CompletedTask);
}

/// <summary>Attach a lifecycle hook that registers a metric before the GET step runs.</summary>
public static object? AttachMetricAwareHookExample()
{
    return baselineScenario().WithClean(context => { context.RegisterMetric(Metric.CreateCounter("orders_seen", "count")); return Task.CompletedTask; });
}
    }
