...
export class WithRunnerKeyMethodReference {
  /** Apply the primary string value shown in the sample method reference. */
  static async ApplyPrimaryValueExample(): Promise<unknown> {
      return LoadStrikeRunner.WithRunnerKey(baseContext(), "runner_dummy_orders_reference");
  }

  /** Show the same method with a second concrete value. */
  static async ApplyAlternateValueExample(): Promise<unknown> {
      return LoadStrikeRunner.WithRunnerKey(baseContext(), "runner_dummy_checkout_reference");
  }
}
