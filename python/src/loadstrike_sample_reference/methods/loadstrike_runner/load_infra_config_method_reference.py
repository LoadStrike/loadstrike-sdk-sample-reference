...
class LoadInfraConfigMethodReference:
    @staticmethod
    def use_generated_config_file_example() -> Any:
        """Point the runner at a temporary JSON file generated inside the sample."""
        paths = write_temp_config_files()
        return base_runner().load_infra_config(paths.infra_path)

    @staticmethod
    def build_context_from_generated_config_example() -> Any:
        """Load the file and continue into a built context."""
        paths = write_temp_config_files()
        return base_runner().load_infra_config(paths.infra_path).build_context()
