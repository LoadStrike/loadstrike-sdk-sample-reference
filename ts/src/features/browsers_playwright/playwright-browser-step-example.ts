...
export async function build(): Promise<unknown> {
  ...
  const browserStep = { scenario: scenario("playwright"), playwrightEntrypoint: chromium, note: "Open a browser inside the scenario step and return a LoadStrikeResponse when navigation completes." };
  const result = await LoadStrikeRunner.registerScenarios(scenario("playwright-browser-step")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("playwright-browser-step").withoutReports().run();
  return { browserStep, result };
}
