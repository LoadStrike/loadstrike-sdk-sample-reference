using System;
    ...
    /// <summary>Declare the expected agent count for distributed runs.</summary>
    public static class WithAgentsCountMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Apply the option with a smaller numeric value.</summary>
public static object? ApplySmallerCountExample()
{
    return LoadStrikeRunner.WithAgentsCount(baseContext(), 2);
}

/// <summary>Apply the option with a larger numeric value.</summary>
public static object? ApplyLargerCountExample()
{
    return LoadStrikeRunner.WithAgentsCount(baseContext(), 4);
}
    }
