...
def build():
    ...
    selectors = {"source_selector": TrackingFieldSelector.parse("header:X-Correlation-Id"), "destination_selector": TrackingFieldSelector.parse("json:$.trackingId"), "source": _http_source(), "destination": _http_destination()}
    result = (LoadStrikeRunner.register_scenarios(_scenario("header-and-json-selectors")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("header-and-json-selectors").without_reports().run())
    return {"selectors": selectors, "Result": result}
