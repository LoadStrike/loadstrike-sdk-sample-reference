...
export async function build(): Promise<unknown> {
  ...
  const browserStep = { scenario: scenario("selenium"), seleniumBuilder: new Builder().forBrowser("chrome"), note: "Build a WebDriver in the step body and close it in a finally block." };
  const result = await LoadStrikeRunner.registerScenarios(scenario("selenium-driver-step")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("selenium-driver-step").withoutReports().run();
