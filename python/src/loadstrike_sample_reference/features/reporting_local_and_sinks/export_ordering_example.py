...
def build():
    ...
    featureBundle = {"report_formats": [LoadStrikeReportFormat.Txt, LoadStrikeReportFormat.Html], "built_in_sinks": _built_in_sinks()}
    result = (LoadStrikeRunner.register_scenarios(_scenario("export-ordering")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("export-ordering").without_reports().run())
