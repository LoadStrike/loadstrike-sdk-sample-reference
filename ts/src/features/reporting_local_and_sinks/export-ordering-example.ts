...
export async function build(): Promise<unknown> {
  ...
  const featureBundle = { reportFormats: [LoadStrikeReportFormat.Txt, LoadStrikeReportFormat.Html], builtInSinks: builtInSinks() };
  const result = await LoadStrikeRunner.registerScenarios(scenario("export-ordering")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("export-ordering").withoutReports().run();
  return { featureBundle, result };
}
