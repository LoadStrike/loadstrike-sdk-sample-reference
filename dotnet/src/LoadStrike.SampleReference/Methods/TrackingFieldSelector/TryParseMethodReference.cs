using System;
    ...
    namespace LoadStrike.SampleReference.Methods.OwnerTrackingFieldSelector;

    /// <summary>Parse a tracking selector without throwing on invalid input.</summary>
    public static class TryParseMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        {
            var parsed = TrackingFieldSelector.TryParse("header:x-id", out var selector);
return new { parsed, selector };
        }
