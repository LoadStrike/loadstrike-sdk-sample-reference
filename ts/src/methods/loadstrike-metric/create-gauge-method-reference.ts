...
export class CreateGaugeMethodReference {
  /** Create the metric directly from the public metric helper. */
  static async CreateMetricExample(): Promise<unknown> {
  ...
  /** Register the metric during scenario init so the GET scenario can report it. */
          static async RegisterMetricDuringInitExample(): Promise<unknown> {
              const metric = LoadStrikeMetric.gauge("orders_latency", "ms");
