...
class AddScenariosMethodReference:
    @staticmethod
    def add_two_scenarios_example() -> Any:
        """Attach two scenarios in a single batch call."""
        return base_runner().add_scenarios(baseline_scenario(), baseline_scenario("orders.audit"))

    @staticmethod
    def add_batch_and_keep_building_example() -> Any:
        """Keep chaining runner options after adding a batch of scenarios."""
        return base_runner().add_scenarios(baseline_scenario(), baseline_scenario("orders.audit")).with_test_name("batched-add")
