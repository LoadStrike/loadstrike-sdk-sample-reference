...
export class WithRuntimePolicyErrorModeMethodReference {
  /** Continue when policy hooks fail. */
  static async ContinueOnPolicyErrorExample(): Promise<unknown> {
      return LoadStrikeRunner.WithRuntimePolicyErrorMode(baseContext(), "continue");
  }

  /** Fail the run when policy hooks fail. */
  static async FailOnPolicyErrorExample(): Promise<unknown> {
      return LoadStrikeRunner.WithRuntimePolicyErrorMode(baseContext(), "fail");
  }
}
