using System;
    ...
    /// <summary>Create a metric-level threshold.</summary>
    public static class CreateMetricMethodReference
    {
...
{
    return LoadStrikeThreshold.CreateMetric(stats => stats.Counters.Any(counter => counter.Value >= 1), 1, TimeSpan.FromSeconds(2));
}

/// <summary>Attach the threshold to the baseline GET scenario.</summary>
public static object? AttachThresholdToScenarioExample()
{
    return baselineScenario().WithThresholds(LoadStrikeThreshold.CreateMetric(stats => stats.Counters.Any(counter => counter.Value >= 1), 1, TimeSpan.FromSeconds(2)));
}
    }
