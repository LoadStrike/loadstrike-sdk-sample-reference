...
export async function build(): Promise<unknown> {
  // Custom Metrics is the feature being explained. The sample returns that focused object alongside a minimal run result.
  const feature = { counter: LoadStrikeMetric.counter("orders_processed", 0, "count"), gauge: LoadStrikeMetric.gauge("queue_depth", 0, "items"), scenario: scenario("metrics") };
  const result = await LoadStrikeRunner.registerScenarios(scenario("custom-metrics")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("custom-metrics").withoutReports().run();
...
export function scenarioWithThreshold(nameSuffix: string, threshold: Record<string, unknown>) {
  return scenario(nameSuffix).withThresholds(threshold);
}
