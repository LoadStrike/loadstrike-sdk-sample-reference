using System;
    ...
    /// <summary>Add multiple scenarios to a reusable runner.</summary>
    public static class AddScenariosMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Attach two scenarios in a single batch call.</summary>
public static object? AddTwoScenariosExample()
{
    return baseRunner().AddScenarios(baselineScenario(), baselineScenario("orders.audit"));
}

/// <summary>Keep chaining runner options after adding a batch of scenarios.</summary>
public static object? AddBatchAndKeepBuildingExample()
{
    return baseRunner().AddScenarios(baselineScenario(), baselineScenario("orders.audit")).WithTestName("batched-add");
}
    }
