...
export class StepPredicateMethodReference {
  /** Create the threshold from the public threshold helper. */
  static async CreateThresholdExample(): Promise<unknown> {
      return LoadStrikeThreshold.stepPredicate("get-order", "step-ok", context => context.runStats.allOkCount >= 1, { abortWhenErrorCount: 1, startCheckAfterSeconds: 2 });
  }

  /** Attach the threshold to the baseline GET scenario. */
  static async AttachThresholdToScenarioExample(): Promise<unknown> {
      return baselineScenario().withThresholds(LoadStrikeThreshold.stepPredicate("get-order", "step-ok", context => context.runStats.allOkCount >= 1, { abortWhenErrorCount: 1, startCheckAfterSeconds: 2 }));
  }
}
