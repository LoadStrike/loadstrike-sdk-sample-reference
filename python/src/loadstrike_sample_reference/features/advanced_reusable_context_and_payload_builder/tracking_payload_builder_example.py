...
FEATURE_OVERVIEW = "This feature area focuses on building runner contexts once and reusing payload or argument helpers without mixing in unrelated transport setup."
...
def build():
    ...
    payloadBuilder = _tracking_builder()
    result = (LoadStrikeRunner.register_scenarios(_scenario("tracking-payload-builder")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("tracking-payload-builder").without_reports().run())
    return {"payloadBuilder": payloadBuilder, "Result": result}
