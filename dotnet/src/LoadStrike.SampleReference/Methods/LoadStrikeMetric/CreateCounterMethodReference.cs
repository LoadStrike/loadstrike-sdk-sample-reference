using System;
    ...
    namespace LoadStrike.SampleReference.Methods.OwnerLoadStrikeMetric;

    /// <summary>Create a counter metric and register it during scenario init.</summary>
    public static class CreateCounterMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Create the metric directly from the public metric helper.</summary>
public static object? CreateMetricExample()
{
    return Metric.CreateCounter("orders_total", "count");
}

/// <summary>Register the metric during scenario init so the GET scenario can report it.</summary>
        public static object? RegisterMetricDuringInitExample()
        {
            var metric = Metric.CreateCounter("orders_total", "count");
var scenario = LoadStrikeScenario.Create("orders.metric", async context => { var reply = await LoadStrikeStep.Run("get-order", context, CreateTypedOrderReplyAsync); return reply.AsReply(); })
    .WithInit(context => { context.RegisterMetric(metric); return Task.CompletedTask; });
return scenario;
        }
    }
