using System;
    ...
    namespace LoadStrike.SampleReference.Methods.OwnerLoadStrikeStep;

    /// <summary>Execute an asynchronous step when that surface is exposed directly.</summary>
    public static class RunAsyncMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Execute the GET request through the explicit async step helper.</summary>
public static object? RunAsyncStepExample()
{
    return LoadStrikeStep.RunAsync("get-order", createStepContext(), CreateTypedOrderReplyAsync);
}

/// <summary>Project the async step reply to the untyped reply contract.</summary>
public static object? RunAsyncStepAndProjectReplyExample()
{
    return LoadStrikeStep.RunAsync("get-order", createStepContext(), CreateTypedOrderReplyAsync).GetAwaiter().GetResult().AsReply();
}
    }
