using System;
    ...
    /// <summary>Add a single scenario to a reusable runner.</summary>
    public static class AddScenarioMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Attach one scenario to a reusable runner.</summary>
public static object? AddOneScenarioExample()
{
    return baseRunner().AddScenario(baselineScenario());
}

/// <summary>Attach a second scenario after the first GET scenario is already registered.</summary>
public static object? AddSecondScenarioExample()
{
    return baseRunner().AddScenario(baselineScenario()).AddScenario(baselineScenario("orders.audit"));
}
    }
