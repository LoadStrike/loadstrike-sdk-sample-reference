...
class ConfigureContextMethodReference:
    @staticmethod
    def merge_built_context_example() -> Any:
        """Merge a previously built context into a new runner."""
        return base_runner().configure_context(base_context())

    @staticmethod
    def merge_tracked_context_example() -> Any:
        """Merge the tracked variant context when the run needs correlation settings."""
        return base_runner().configure_context(tracked_context())
