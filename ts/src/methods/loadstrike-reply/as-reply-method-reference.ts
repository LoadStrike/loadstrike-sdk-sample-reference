...
export class AsReplyMethodReference {
  /** Project a typed reply into the untyped reply contract. */
  static async ProjectTypedReplyExample(): Promise<unknown> {
      return (() => { const reply = LoadStrikeResponse.ok({ orderId: 42 }, "200", 128, "ok", 3.2); return reply.asReply?.() ?? reply; })();
  }

  /** Project a failed typed reply into the untyped reply contract. */
  static async ProjectFailedTypedReplyExample(): Promise<unknown> {
      return (() => { const reply = LoadStrikeResponse.fail({ error: "not-found" }, "404", 64, "missing", 3.2); return reply.asReply?.() ?? reply; })();
  }
}
