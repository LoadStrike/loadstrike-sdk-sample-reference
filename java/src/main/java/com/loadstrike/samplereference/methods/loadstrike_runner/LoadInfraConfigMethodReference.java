package com.loadstrike.samplereference.methods.loadstrike_runner;
      ...
      public final class LoadInfraConfigMethodReference {
        private LoadInfraConfigMethodReference() {}
...
return baseRunner().loadInfraConfig(paths.InfraPath);
        }

/** Load the file and continue into a built context. */
        public static Object BuildContextFromGeneratedConfigExample() {
            var paths = writeTempConfigFiles();
return baseRunner().loadInfraConfig(paths.InfraPath).buildContext();
        }
      }
