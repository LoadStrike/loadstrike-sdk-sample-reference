using System;
    ...
    /// <summary>Patch runner options through a callback-based configurator.</summary>
    public static class ConfigureMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Patch the runner through a callback that adjusts the suite label.</summary>
public static object? ConfigureSuiteExample()
{
    return baseRunner().Configure(context => context.WithTestSuite("configured-suite"));
}

/// <summary>Reuse the configurator hook for a different context option.</summary>
public static object? ConfigureSessionExample()
{
    return baseRunner().Configure(context => context.WithSessionId("orders-configured-session"));
}
    }
