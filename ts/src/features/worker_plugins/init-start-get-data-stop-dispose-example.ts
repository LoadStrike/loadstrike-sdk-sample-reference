...
/**
 * This feature area focuses on the worker-plugin lifecycle, registration, and custom plugin tables in final results.
 */
export async function build(): Promise<unknown> {
  // Init Start Get Data Stop Dispose is the feature being explained. The sample returns that focused object alongside a minimal run result.
  const plugin = new OrdersWorkerPlugin();
  const result = await LoadStrikeRunner.registerScenarios(scenario("init-start-get-data-stop-dispose")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("init-start-get-data-stop-dispose").withoutReports().run();
...
export class OrdersWorkerPlugin {
  ...
  getData(_result: LoadStrikeRunResult): LoadStrikePluginData {
    const table = LoadStrikePluginDataTable.create("Captured Orders");
    table.headers = ["Order Number", "Tenant"];
    table.rows = [
      { "Order Number": EXAMPLE_ORDER_NUMBER, Tenant: EXAMPLE_TENANT },
    ];

    const pluginData = LoadStrikePluginData.create(this.pluginName);
    pluginData.hints = [
      "Reference-only plugin payload for the sample repository.",
    ];
    pluginData.tables = [table];
    return pluginData;
  }
}
