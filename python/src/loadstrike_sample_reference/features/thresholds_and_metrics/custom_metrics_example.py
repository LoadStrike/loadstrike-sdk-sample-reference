...
def build():
    ...
    feature = {"counter": LoadStrikeMetric.counter("orders_processed", "count", initial_value=0), "gauge": LoadStrikeMetric.gauge("queue_depth", "items", initial_value=0), "scenario": _scenario("metrics")}
    result = (LoadStrikeRunner.register_scenarios(_scenario("custom-metrics")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("custom-metrics").without_reports().run())
