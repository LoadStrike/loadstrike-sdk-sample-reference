...
export class OkWithPayloadMethodReference {
  /** Create the primary reply shape for this helper. */
  static async CreatePrimaryReplyExample(): Promise<unknown> {
      return LoadStrikeResponse.okWithPayload({ orderId: "ORD-10001" }, "202", 128, "accepted", 3.2);
  }

  /** Create an alternate reply shape that changes status or message metadata. */
  static async CreateAlternateReplyExample(): Promise<unknown> {
      return LoadStrikeResponse.okWithPayload({ orderId: "ORD-10002" }, "201", 64, "created", 4.6);
  }
}
