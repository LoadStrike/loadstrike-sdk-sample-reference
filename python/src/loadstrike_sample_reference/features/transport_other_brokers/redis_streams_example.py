...
def build():
    ...
    endpoints = _other_brokers()["redis"]
    result = (LoadStrikeRunner.register_scenarios(_scenario("redis-streams")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("redis-streams").without_reports().run())
