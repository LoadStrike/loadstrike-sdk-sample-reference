...
export async function build(): Promise<unknown> {
  // Inject keeps the scenario setup focused on one behavior. Change the option below first when comparing suboptions.
  const scenario = scenarioWithSimulation("inject", LoadStrikeSimulation.inject(4, 0.25, 20));
  return await LoadStrikeRunner.registerScenarios(scenario).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("inject").withoutReports().run();
}
