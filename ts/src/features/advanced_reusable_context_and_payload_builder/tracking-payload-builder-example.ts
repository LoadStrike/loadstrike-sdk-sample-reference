...
export async function build(): Promise<unknown> {
  ...
  const payloadBuilder = trackingBuilder();
  const result = await LoadStrikeRunner.registerScenarios(scenario("tracking-payload-builder")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("tracking-payload-builder").withoutReports().run();
  return { payloadBuilder, result };
}
