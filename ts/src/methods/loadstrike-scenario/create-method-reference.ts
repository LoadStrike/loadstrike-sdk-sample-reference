...
export class CreateMethodReference {
  ...
  static async CreateBaselineScenarioExample(): Promise<unknown> {
      return LoadStrikeScenario.create("orders.get-by-id", executeOrderGet);
  }

  /** Create the baseline scenario and immediately add a load simulation. */
  static async CreateScenarioAndAddSimulationExample(): Promise<unknown> {
      return LoadStrikeScenario.create("orders.get-by-id", executeOrderGet).withLoadSimulations(LoadStrikeSimulation.iterationsForConstant(1, 3));
  }
}
