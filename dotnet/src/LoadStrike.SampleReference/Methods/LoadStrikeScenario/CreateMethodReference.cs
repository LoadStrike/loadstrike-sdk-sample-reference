using System;
    ...
    /// <summary>Create a synchronous scenario around a basic GET request step.</summary>
    public static class CreateMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Create the default GET /orders/{id} scenario.</summary>
public static object? CreateBaselineScenarioExample()
{
    return LoadStrikeScenario.Create("orders.get-by-id", ExecuteOrderGetAsync);
}

/// <summary>Create the baseline scenario and immediately add a load simulation.</summary>
public static object? CreateScenarioAndAddSimulationExample()
{
    return LoadStrikeScenario.Create("orders.get-by-id", ExecuteOrderGetAsync).WithLoadSimulations(LoadStrikeSimulation.IterationsForConstant(1, 3));
}
    }
