...
export class MetricPredicateMethodReference {
  /** Create the threshold from the public threshold helper. */
  static async CreateThresholdExample(): Promise<unknown> {
      return LoadStrikeThreshold.metricPredicate("metric-ok", () => true, { abortWhenErrorCount: 1, startCheckAfterSeconds: 2 });
  }

  /** Attach the threshold to the baseline GET scenario. */
  static async AttachThresholdToScenarioExample(): Promise<unknown> {
      return baselineScenario().withThresholds(LoadStrikeThreshold.metricPredicate("metric-ok", () => true, { abortWhenErrorCount: 1, startCheckAfterSeconds: 2 }));
  }
}
