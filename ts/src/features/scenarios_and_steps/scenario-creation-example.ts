...
export async function build(): Promise<unknown> {
  ...
  const feature = scenario("create");
  const result = await LoadStrikeRunner.registerScenarios(scenario("scenario-creation")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("scenario-creation").withoutReports().run();
