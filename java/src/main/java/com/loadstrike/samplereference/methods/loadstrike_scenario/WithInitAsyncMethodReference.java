package com.loadstrike.samplereference.methods.loadstrike_scenario;
      ...
      public final class WithInitAsyncMethodReference {
        private WithInitAsyncMethodReference() {}
...
public static Object AttachAsyncNoOpHookExample() {
    return baselineScenario().withInitAsync(context -> CompletableFuture.completedFuture(null));
}

/** Attach an asynchronous lifecycle hook that still registers a metric. */
public static Object AttachAsyncMetricHookExample() {
    return baselineScenario().withInitAsync(context -> CompletableFuture.runAsync(() -> context.registerMetric(LoadStrikeMetric.counter("orders_seen", "count"))));
}
      }
