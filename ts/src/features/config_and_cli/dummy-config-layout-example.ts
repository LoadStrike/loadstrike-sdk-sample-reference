...
export async function build(): Promise<unknown> {
  ...
  const configLayout = { configFile: "loadstrike.config.json", infraFile: "loadstrike.infra.json", runnerKey: RUNNER_KEY };
  const context = LoadStrikeRunner.registerScenarios(scenario("config-layout")).loadConfig(`features/config_and_cli/${configLayout.configFile}`).loadInfraConfig(`features/config_and_cli/${configLayout.infraFile}`).withRunnerKey(configLayout.runnerKey).buildContext();
