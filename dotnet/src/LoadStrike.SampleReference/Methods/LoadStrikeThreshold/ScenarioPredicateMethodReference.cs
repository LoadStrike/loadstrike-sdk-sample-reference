using System;
    ...
    /// <summary>Create a scenario predicate threshold where predicate helpers exist.</summary>
    public static class ScenarioPredicateMethodReference
    {
...
{
    return LoadStrikeThreshold.ScenarioPredicate(stats => stats.AllOkCount >= 1, 1, TimeSpan.FromSeconds(2));
}

/// <summary>Attach the threshold to the baseline GET scenario.</summary>
public static object? AttachThresholdToScenarioExample()
{
    return baselineScenario().WithThresholds(LoadStrikeThreshold.ScenarioPredicate(stats => stats.AllOkCount >= 1, 1, TimeSpan.FromSeconds(2)));
}
    }
