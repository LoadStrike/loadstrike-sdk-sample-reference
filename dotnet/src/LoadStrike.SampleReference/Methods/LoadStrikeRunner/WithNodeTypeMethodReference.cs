using System;
    ...
    /// <summary>Choose the node role for the current process.</summary>
    public static class WithNodeTypeMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Set the runner to coordinator mode.</summary>
public static object? UseCoordinatorNodeExample()
{
    return LoadStrikeRunner.WithNodeType(baseContext(), LoadStrikeNodeType.Coordinator);
}

/// <summary>Set the runner to agent mode.</summary>
public static object? UseAgentNodeExample()
{
    return LoadStrikeRunner.WithNodeType(baseContext(), LoadStrikeNodeType.Agent);
}
    }
