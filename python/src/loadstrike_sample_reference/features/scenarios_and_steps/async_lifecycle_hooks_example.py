...
def build():
    ...
    feature = _scenario("async-lifecycle")
    result = (LoadStrikeRunner.register_scenarios(_scenario("async-lifecycle-hooks")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("async-lifecycle-hooks").without_reports().run())
