...
export async function build(): Promise<unknown> {
  // Built In Plugin Auto Registration is configured on a reusable context so you can compare the context flow separately from scenario authoring.
  const context = runner().buildContext().withWorkerPlugins(new OrdersWorkerPlugin());
