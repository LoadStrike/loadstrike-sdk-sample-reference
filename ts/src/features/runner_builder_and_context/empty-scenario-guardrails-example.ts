...
export async function build(): Promise<unknown> {
  ...
  const scenario = LoadStrikeScenario.empty(scenarioName("empty"));
  return await LoadStrikeRunner.registerScenarios(scenario).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("empty-scenario-guardrails").withoutReports().run();
}
