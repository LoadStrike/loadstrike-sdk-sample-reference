...
def build():
    ...
    scenario = _scenario_with_simulation("inject-random", LoadStrikeSimulation.inject_random(1, 8, 0.25, 20))
    return (LoadStrikeRunner.register_scenarios(scenario).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("inject-random").without_reports().run())
