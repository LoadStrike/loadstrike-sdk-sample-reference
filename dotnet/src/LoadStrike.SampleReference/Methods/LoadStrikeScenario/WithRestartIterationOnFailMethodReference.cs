using System;
    ...
    /// <summary>Retry scenario iterations when the reply fails.</summary>
    public static class WithRestartIterationOnFailMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Retry iterations when the GET step fails.</summary>
public static object? EnableRestartOnFailExample()
{
    return baselineScenario().WithRestartIterationOnFail(true);
}

/// <summary>Leave iteration restart disabled.</summary>
public static object? DisableRestartOnFailExample()
{
    return baselineScenario().WithRestartIterationOnFail(false);
}
    }
