...
class WithInitAsyncMethodReference:
    @staticmethod
    def attach_async_no_op_hook_example() -> Any:
        """Attach an asynchronous lifecycle hook with no extra side effects."""
        return baseline_scenario().with_init_async(lambda context: None)

    @staticmethod
    def attach_async_metric_hook_example() -> Any:
        """Attach an asynchronous lifecycle hook that still registers a metric."""
        return baseline_scenario().with_init_async(lambda context: context.register_metric(LoadStrikeMetric.counter("orders_seen", "count")))
