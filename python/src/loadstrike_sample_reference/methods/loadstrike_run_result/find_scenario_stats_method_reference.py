...
class FindScenarioStatsMethodReference:
    @staticmethod
    def read_run_results_example() -> Any:
        """Read data back from the run result surface with this helper."""
        return run_result().find_scenario_stats("orders.get-by-id")

    @staticmethod
    def read_deeper_result_surface_example() -> Any:
        """Continue into the scenario or step stats surface after the first lookup."""
        return run_result().get_scenario_stats("orders.get-by-id")
