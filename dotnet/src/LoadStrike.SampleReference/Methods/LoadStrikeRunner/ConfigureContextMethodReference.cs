using System;
    ...
    /// <summary>Merge an existing context into a runner where supported.</summary>
    public static class ConfigureContextMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Merge a previously built context into a new runner.</summary>
public static object? MergeBuiltContextExample()
{
    return baseRunner().ConfigureContext(baseContext());
}

/// <summary>Merge the tracked variant context when the run needs correlation settings.</summary>
public static object? MergeTrackedContextExample()
{
    return baseRunner().ConfigureContext(trackedContext());
}
    }
