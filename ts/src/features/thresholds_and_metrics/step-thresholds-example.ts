...
export async function build(): Promise<unknown> {
  ...
  const scenario = scenarioWithThreshold("step-threshold", LoadStrikeThreshold.CreateStep("publish-order", () => true, 1));
  return await LoadStrikeRunner.registerScenarios(scenario).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("step-thresholds").withoutReports().run();
}
