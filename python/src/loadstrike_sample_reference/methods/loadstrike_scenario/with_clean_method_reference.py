...
class WithCleanMethodReference:
    @staticmethod
    def attach_no_op_hook_example() -> Any:
        """Attach a no-op lifecycle hook for the baseline scenario."""
        return baseline_scenario().with_clean(lambda context: None)

    @staticmethod
    def attach_metric_aware_hook_example() -> Any:
        """Attach a lifecycle hook that registers a metric before the GET step runs."""
        return baseline_scenario().with_clean(lambda context: context.register_metric(LoadStrikeMetric.counter("orders_seen", "count")))
