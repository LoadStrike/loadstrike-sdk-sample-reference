...
export async function build(): Promise<unknown> {
  // Load Json Config is the main thing being demonstrated here. Keep the runner flow simple and change only this option when comparing behavior.
  const sampleRunner = runner().loadConfig("./config/loadstrike.config.json").loadInfraConfig("./config/loadstrike.infra.json");
