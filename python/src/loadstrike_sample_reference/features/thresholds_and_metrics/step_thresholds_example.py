...
def build():
    ...
    scenario = _scenario_with_threshold("step-threshold", LoadStrikeThreshold.create_step("publish-order", lambda stats: True, abort_when_error_count=1))
    return (LoadStrikeRunner.register_scenarios(scenario).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("step-thresholds").without_reports().run())
