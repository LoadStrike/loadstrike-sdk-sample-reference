...
def build():
    ...
    scenario = _scenario_with_simulation("ramping-constant", LoadStrikeSimulation.ramping_constant(5, 30))
    return (LoadStrikeRunner.register_scenarios(scenario).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("ramping-constant").without_reports().run())
