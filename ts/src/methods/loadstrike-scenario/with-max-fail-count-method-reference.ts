...
export class WithMaxFailCountMethodReference {
  /** Stop the scenario after the first failure. */
  static async StopAfterOneFailureExample(): Promise<unknown> {
      return baselineScenario().withMaxFailCount(1);
  }

  /** Allow a few failures before stopping the scenario. */
  static async StopAfterThreeFailuresExample(): Promise<unknown> {
