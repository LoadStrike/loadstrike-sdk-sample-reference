...
func runWriteTempConfigFiles() runTempConfigPaths {
	return runTempConfigPaths{
        ...
        // Execute the baseline GET request inside a named step.
func (reference RunMethodReference) RunNamedStepExample() any {
    return loadstrike.CreateScenario("orders.step", func(context loadstrike.LoadStrikeScenarioContext) loadstrike.LoadStrikeReply { return loadstrike.LoadStrikeStep.Run("get-order", context, func(loadstrike.LoadStrikeScenarioContext) loadstrike.LoadStrikeReply { return runPerformOrderGetReply() }) })
}

// Project the typed step reply back into the untyped reply surface.
func (reference RunMethodReference) RunStepAndProjectReplyExample() any {
    return loadstrike.CreateScenario("orders.step", func(context loadstrike.LoadStrikeScenarioContext) loadstrike.LoadStrikeReply { return loadstrike.LoadStrikeStep.Run("get-order", context, func(loadstrike.LoadStrikeScenarioContext) loadstrike.LoadStrikeReply { return runPerformOrderGetReply() }) })
}
