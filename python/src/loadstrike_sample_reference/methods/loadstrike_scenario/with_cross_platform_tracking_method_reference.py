...
class WithCrossPlatformTrackingMethodReference:
    @staticmethod
    def attach_delegate_tracking_example() -> Any:
        """Attach the correlation configuration used for source and destination tracking."""
        return baseline_scenario().with_cross_platform_tracking(tracking_configuration())

    @staticmethod
    def attach_tracking_to_empty_scenario_example() -> Any:
        """Start from an empty scenario and attach cross-platform tracking."""
        return LoadStrikeScenario.empty("orders.empty").with_cross_platform_tracking(tracking_configuration())
