...
export async function build(): Promise<unknown> {
  ...
  const endpoints = otherBrokers().rabbitmq;
  const result = await LoadStrikeRunner.registerScenarios(scenario("rabbitmq")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("rabbitmq").withoutReports().run();
