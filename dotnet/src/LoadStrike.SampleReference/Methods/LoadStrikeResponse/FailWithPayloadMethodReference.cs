using System;
    ...
    /// <summary>Return a failed typed payload reply.</summary>
    public static class FailWithPayloadMethodReference
    {
...
{
    return LoadStrikeResponse.Fail(payload: new Dictionary<string, object>(StringComparer.Ordinal) { ["error"] = "not-found" }, statusCode: "409", message: "conflict", sizeBytes: 64, customLatencyMs: 3.2);
}
...
{
    return LoadStrikeResponse.Fail(payload: new Dictionary<string, object>(StringComparer.Ordinal) { ["error"] = "timeout" }, statusCode: "504", message: "retry", sizeBytes: 8, customLatencyMs: 4.8);
}
    }
