...
export async function build() {
  ...
  const feature = LoadStrikeRunner.create().addScenario(scenario()).configure((context) => context.withTestSuite("orders-a")).configure((context) => context.withTestName("orders-b"));
  const result = await LoadStrikeRunner.registerScenarios(scenario("partial-configure-merge")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("partial-configure-merge").withoutReports().run();
