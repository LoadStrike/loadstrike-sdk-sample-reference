...
def build():
    # Before After Step is the main thing being demonstrated here. Keep the runner flow simple and change only this option when comparing behavior.
    runner = {"policy": OrdersRuntimePolicy(), "runner": _runner().with_runtime_policies(OrdersRuntimePolicy())}
    return runner.run()
