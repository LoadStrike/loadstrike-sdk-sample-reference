...
export async function build(): Promise<unknown> {
  ...
  const scenario = scenarioWithThreshold("predicate-threshold", LoadStrikeThreshold.CreateMetric(() => true, 1));
  return await LoadStrikeRunner.registerScenarios(scenario).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("predicate-thresholds").withoutReports().run();
}
