...
export class GetScenarioTimerTimeMethodReference {
  /** Call the public context helper directly from the scenario context surface. */
  static async UseContextMethodExample() {
      return LoadStrikeScenario.create("orders.timer", context => { const ignored = context.getScenarioTimerTime(); return LoadStrikeResponse.ok(); });
  }

  /** Show the same helper in the baseline GET-step flow. */
  static async UseContextMethodInStepExample() {
      return LoadStrikeScenario.create("orders.timer", context => { const ignored = context.getScenarioTimerTime(); return LoadStrikeResponse.ok(); });
  }
}
