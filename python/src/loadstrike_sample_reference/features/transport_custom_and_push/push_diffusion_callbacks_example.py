...
def build():
    ...
    endpoints = _custom_endpoints()["push_diffusion"]
    result = (LoadStrikeRunner.register_scenarios(_scenario("push-diffusion-callbacks")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("push-diffusion-callbacks").without_reports().run())
