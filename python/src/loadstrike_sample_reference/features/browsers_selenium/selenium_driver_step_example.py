...
FEATURE_OVERVIEW = "This feature area keeps the scenario small so the Selenium driver step is the main thing being explained."
...
def build():
    ...
    browserStep = {"scenario": _scenario("selenium"), "chrome_options": ChromeOptions(), "note": "Build a WebDriver in the step body and close it in a finally block."}
    result = (LoadStrikeRunner.register_scenarios(_scenario("selenium-driver-step")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("selenium-driver-step").without_reports().run())
