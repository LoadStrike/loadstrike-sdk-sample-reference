...
class ConfigureMethodReference:
    @staticmethod
    def configure_suite_example() -> Any:
        """Patch the runner through a callback that adjusts the suite label."""
        return base_runner().configure(lambda context: context.with_test_suite("configured-suite"))

    @staticmethod
    def configure_session_example() -> Any:
        """Reuse the configurator hook for a different context option."""
        return base_runner().configure(lambda context: context.with_session_id("orders-configured-session"))
