...
export class RunAsyncMethodReference {
  /** Execute the GET request through the explicit async step helper. */
  static async RunAsyncStepExample(): Promise<unknown> {
      return LoadStrikeStep.runAsync("get-order", createStepContext(), async () => createOrderReply());
  }

  /** Project the async step reply to the untyped reply contract. */
  static async RunAsyncStepAndProjectReplyExample(): Promise<unknown> {
      return LoadStrikeStep.runAsync("get-order", createStepContext(), async () => createOrderReply()).then(reply => reply.asReply());
  }
}
