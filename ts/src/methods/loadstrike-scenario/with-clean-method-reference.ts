...
export class WithCleanMethodReference {
  /** Attach a no-op lifecycle hook for the baseline scenario. */
  static async AttachNoOpHookExample(): Promise<unknown> {
      return baselineScenario().withClean(context => undefined);
  }

  /** Attach a lifecycle hook that registers a metric before the GET step runs. */
  static async AttachMetricAwareHookExample(): Promise<unknown> {
      return baselineScenario().withClean(context => context.registerMetric(LoadStrikeMetric.counter("orders_seen", "count")));
  }
}
