...
export class CreateAsyncMethodReference {
  /** Create the GET scenario from an asynchronous callback. */
  static async CreateAsyncScenarioExample(): Promise<unknown> {
      return LoadStrikeScenario.createAsync("orders.get-by-id", async context => executeOrderGet(context));
  }

  /** Create the async scenario and then attach a simple simulation. */
  static async CreateAsyncScenarioWithSimulationExample(): Promise<unknown> {
      return LoadStrikeScenario.createAsync("orders.get-by-id", async context => executeOrderGet(context)).withLoadSimulations(LoadStrikeSimulation.iterationsForConstant(1, 3));
  }
}
