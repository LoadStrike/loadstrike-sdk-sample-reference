...
export async function build() {
  // Local Dev Cluster is configured on a reusable context so you can compare the context flow separately from scenario authoring.
  const feature = { localClusterContext: LoadStrikeRunner.EnableLocalDevCluster(runner().buildContext(), true), natsClusterContext: LoadStrikeRunner.WithNatsServerUrl(runner().buildContext(), NATS_SERVER_URL) };
  const result = await LoadStrikeRunner.Run(feature.localClusterContext);
