...
class WithLoggerConfigMethodReference:
    @staticmethod
    def attach_inline_logger_factory_example() -> Any:
        """Attach a logger factory directly on the runner."""
        return base_runner().with_logger_config(lambda: {"logger": "orders"})

    @staticmethod
    def attach_logger_before_context_build_example() -> Any:
        """Attach the logger factory before building a reusable context."""
        return base_runner().with_logger_config(lambda: {"logger": "orders"}).build_context()
