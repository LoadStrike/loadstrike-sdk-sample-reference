...
FEATURE_OVERVIEW = "This feature area focuses on should-run filters, lifecycle callbacks, error modes, and the result surface for runtime policy failures."


def build():
    # Should Run Scenario is the feature being explained. The sample returns that focused object alongside a minimal run result.
    policy = OrdersRuntimePolicy()
    result = (LoadStrikeRunner.register_scenarios(_scenario("should-run-scenario")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("should-run-scenario").without_reports().run())
