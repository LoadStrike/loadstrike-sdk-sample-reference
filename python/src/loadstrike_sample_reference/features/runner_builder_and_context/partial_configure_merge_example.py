...
FEATURE_OVERVIEW = "This feature area focuses on creating runners, reusing built contexts, and merging partial configuration."


def build():
    # Partial Configure Merge is the feature being explained. The sample returns that focused object alongside a minimal run result.
    feature = LoadStrikeRunner.create().add_scenario(_scenario()).configure(lambda context: context.with_test_suite("orders-a")).configure(lambda context: context.with_test_name("orders-b"))
    result = (LoadStrikeRunner.register_scenarios(_scenario("partial-configure-merge")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("partial-configure-merge").without_reports().run())
