using System;
    ...
    namespace LoadStrike.SampleReference.Methods.OwnerLoadStrikeScenario;

    /// <summary>Attach scenario or step thresholds.</summary>
    public static class WithThresholdsMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Attach one scenario-level threshold.</summary>
public static object? AttachScenarioThresholdExample()
{
    return baselineScenario().WithThresholds(LoadStrikeThreshold.CreateScenario(stats => stats.AllRequestCount >= 1, 1, TimeSpan.FromSeconds(2)));
}

/// <summary>Attach more than one threshold in the same call.</summary>
public static object? AttachScenarioAndStepThresholdExample()
{
    return baselineScenario().WithThresholds(LoadStrikeThreshold.CreateScenario(stats => stats.AllRequestCount >= 1, 1, TimeSpan.FromSeconds(2)), LoadStrikeThreshold.CreateStep("get-order", stats => stats.Ok.Request.Count >= 1, 1, TimeSpan.FromSeconds(2)));
}
    }
