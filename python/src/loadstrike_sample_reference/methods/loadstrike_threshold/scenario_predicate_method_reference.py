...
class ScenarioPredicateMethodReference:
    @staticmethod
    def create_threshold_example() -> Any:
        """Create the threshold from the public threshold helper."""
        return LoadStrikeThreshold.scenario_predicate("scenario-ok", lambda stats: stats.all_ok_count >= 1, abort_when_error_count=1)

    @staticmethod
    def attach_threshold_to_scenario_example() -> Any:
        """Attach the threshold to the baseline GET scenario."""
        return baseline_scenario().with_thresholds(LoadStrikeThreshold.scenario_predicate("scenario-ok", lambda stats: stats.all_ok_count >= 1, abort_when_error_count=1))
