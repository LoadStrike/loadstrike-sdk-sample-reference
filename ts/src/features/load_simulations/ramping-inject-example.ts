...
export async function build(): Promise<unknown> {
  ...
  const scenario = scenarioWithSimulation("ramping-inject", LoadStrikeSimulation.rampingInject(6, 0.2, 30));
  return await LoadStrikeRunner.registerScenarios(scenario).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("ramping-inject").withoutReports().run();
}
