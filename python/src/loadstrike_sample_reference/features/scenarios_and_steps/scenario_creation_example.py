...
def build():
    ...
    feature = _scenario("create")
    result = (LoadStrikeRunner.register_scenarios(_scenario("scenario-creation")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("scenario-creation").without_reports().run())
