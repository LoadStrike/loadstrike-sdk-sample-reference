...
def build():
    ...
    scenario = _scenario_with_threshold("scenario-threshold", LoadStrikeThreshold.create_scenario(lambda stats: True, abort_when_error_count=1))
    return (LoadStrikeRunner.register_scenarios(scenario).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("scenario-thresholds").without_reports().run())
