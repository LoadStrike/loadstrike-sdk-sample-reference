...
FEATURE_OVERVIEW = "This feature area focuses on the worker-plugin lifecycle, registration, and custom plugin tables in final results."
...
def build():
    ...
    plugin = OrdersWorkerPlugin()
    result = (LoadStrikeRunner.register_scenarios(_scenario("init-start-get-data-stop-dispose")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("init-start-get-data-stop-dispose").without_reports().run())
