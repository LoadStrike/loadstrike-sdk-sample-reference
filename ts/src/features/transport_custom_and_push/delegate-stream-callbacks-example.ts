...
export async function build(): Promise<unknown> {
  ...
  const endpoints = customEndpoints().delegateStream;
  const result = await LoadStrikeRunner.registerScenarios(scenario("delegate-stream-callbacks")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("delegate-stream-callbacks").withoutReports().run();
...
export function customEndpoints() {
  return {
    delegateStream: new DelegateStreamEndpointDefinition({
      ...
      ConsumeAsync: async () => ({ headers: { "x-id": "delegate-track" } }),
    }),
    pushDiffusion: new PushDiffusionEndpointDefinition({
      ...
      SubscribeAsync: async () => ({ headers: { "x-id": "push-track" } }),
    }),
  };
}
