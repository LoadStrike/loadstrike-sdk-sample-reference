...
export async function build(): Promise<unknown> {
  ...
  const feature = { source: httpSource(), destination: httpDestination() };
  const result = await LoadStrikeRunner.registerScenarios(scenario("http-source-and-destination")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("http-source-and-destination").withoutReports().run();
