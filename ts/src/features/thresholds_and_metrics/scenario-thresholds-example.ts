...
export async function build(): Promise<unknown> {
  ...
  const scenario = scenarioWithThreshold("scenario-threshold", LoadStrikeThreshold.CreateScenario(() => true, 1));
  return await LoadStrikeRunner.registerScenarios(scenario).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("scenario-thresholds").withoutReports().run();
}
