...
export async function build() {
  ...
  const scenario = trackedScenario();
  return await LoadStrikeRunner.registerScenarios(scenario).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("cross-platform-scenario-config").withoutReports().run();
}
