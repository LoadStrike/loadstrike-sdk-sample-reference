...
export async function build() {
  ...
  const endpoints = kafkaPair().producer;
  const result = await LoadStrikeRunner.registerScenarios(scenario("security-and-sasl")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("security-and-sasl").withoutReports().run();
