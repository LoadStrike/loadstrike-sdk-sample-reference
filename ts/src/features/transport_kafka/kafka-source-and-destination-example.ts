...
export async function build(): Promise<unknown> {
  ...
  const endpoints = { producer: kafkaPair().producer, consumer: kafkaPair().consumer };
  const result = await LoadStrikeRunner.registerScenarios(scenario("kafka-source-and-destination")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("kafka-source-and-destination").withoutReports().run();
