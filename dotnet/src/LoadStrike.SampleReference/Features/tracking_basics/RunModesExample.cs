...
namespace LoadStrike.SampleReference.Features.TrackingBasics;
...
/// </summary>
public static class RunModesExample
{
    public static object Build()
    {
        // Run Modes is the feature being explained. The sample returns that focused object alongside a minimal run result.
        var runModes = new
                            {
                                GenerateAndCorrelate = TrackingRunMode.GenerateAndCorrelate,
                                CorrelateExistingTraffic = TrackingRunMode.CorrelateExistingTraffic,
                            };
        var result = LoadStrikeRunner
            .RegisterScenarios(Scenario("run-modes"))
            .WithRunnerKey(OrdersWorkflowPlaceholders.RunnerKey)
            .WithTestSuite("orders-reference")
            .WithTestName("run-modes")
            .WithoutReports()
            .Run();

        return new { runModes, Result = result };
    }
...

        {
...

            {
...

                {
                    ...
                    CorrelationTimeout = TimeSpan.FromSeconds(45),

                    TimeoutSweepInterval = TimeSpan.FromMilliseconds(500),

                    TimeoutBatchSize = 100,

                    TimeoutCountsAsFailure = true,

                    TrackingFieldValueCaseSensitive = true,

                    GatherByFieldValueCaseSensitive = true,
