...
export async function build(): Promise<unknown> {
  // Coordinator Agent Over NATS is configured on a reusable context so you can compare the context flow separately from scenario authoring.
  const feature = { context: LoadStrikeRunner.WithAgentsCount(LoadStrikeRunner.WithAgentGroup(LoadStrikeRunner.WithClusterId(LoadStrikeRunner.WithNatsServerUrl(runner().buildContext(), NATS_SERVER_URL), "orders-cluster"), "orders-agents"), 3), coordinatorTargetScenarios: ["orders.publish"] };
  const result = await LoadStrikeRunner.Run(feature.context);
