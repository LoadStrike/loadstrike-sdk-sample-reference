...
export class WithWorkerPluginsMethodReference {
  /** Attach one worker plugin instance. */
  static async AttachOneWorkerPluginExample(): Promise<unknown> {
      return LoadStrikeRunner.WithWorkerPlugins(baseContext(), new OrdersWorkerPlugin());
  }

  /** Attach multiple worker plugins in one call. */
  static async AttachTwoWorkerPluginsExample(): Promise<unknown> {
