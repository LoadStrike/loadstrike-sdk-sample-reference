...
export async function build(): Promise<unknown> {
  ...
  const result = await LoadStrikeRunner.registerScenarios(scenario("response-source-tracking")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("response-source-tracking").withoutReports().run();
