...
export async function build(): Promise<unknown> {
  // Scenario Partition Assignment is configured on a reusable context so you can compare the context flow separately from scenario authoring.
  const feature = { context: LoadStrikeRunner.WithClusterId(runner().buildContext(), "orders-cluster"), partitionAssignment: { "orders.publish": 0, "orders.verify": 1 } };
