using System;
    ...
    namespace LoadStrike.SampleReference.Methods.OwnerLoadStrikeThreshold;

    /// <summary>Create a metric predicate threshold where predicate helpers exist.</summary>
    public static class MetricPredicateMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Create the threshold from the public threshold helper.</summary>
public static object? CreateThresholdExample()
{
    return LoadStrikeThreshold.MetricPredicate(stats => stats.Counters.Any(counter => counter.Value >= 1), 1, TimeSpan.FromSeconds(2));
}

/// <summary>Attach the threshold to the baseline GET scenario.</summary>
public static object? AttachThresholdToScenarioExample()
{
    return baselineScenario().WithThresholds(LoadStrikeThreshold.MetricPredicate(stats => stats.Counters.Any(counter => counter.Value >= 1), 1, TimeSpan.FromSeconds(2)));
}
    }
