...
export class BuildContextMethodReference {
  /** Freeze the fluent runner state into a reusable context. */
  static async BuildReusableContextExample(): Promise<unknown> {
      return baseRunner().buildContext();
  }

  /** Build a context after overriding the sample test name. */
  static async BuildContextAfterRenamingExample(): Promise<unknown> {
