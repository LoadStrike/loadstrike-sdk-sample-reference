using System;
    ...
    /// <summary>Create a step-level threshold.</summary>
    public static class CreateStepMethodReference
    {
...
{
    return LoadStrikeThreshold.CreateStep("get-order", stats => stats.Ok.Request.Count >= 1, 1, TimeSpan.FromSeconds(2));
}

/// <summary>Attach the threshold to the baseline GET scenario.</summary>
public static object? AttachThresholdToScenarioExample()
{
    return baselineScenario().WithThresholds(LoadStrikeThreshold.CreateStep("get-order", stats => stats.Ok.Request.Count >= 1, 1, TimeSpan.FromSeconds(2)));
}
    }
