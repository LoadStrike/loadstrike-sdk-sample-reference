...
def build():
    ...
    endpoints = {"producer": _kafka_pair()[0], "consumer": _kafka_pair()[1], "consumer_group_id": "orders-sample-group"}
    result = (LoadStrikeRunner.register_scenarios(_scenario("consumer-groups")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("consumer-groups").without_reports().run())
