...
def build():
    ...
    endpoints = _other_brokers()["event_hubs"]
    result = (LoadStrikeRunner.register_scenarios(_scenario("azure-event-hubs")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("azure-event-hubs").without_reports().run())
