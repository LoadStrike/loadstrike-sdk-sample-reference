package com.loadstrike.samplereference.methods.loadstrike_threshold;
      ...
      public final class MetricPredicateMethodReference {
        private MetricPredicateMethodReference() {}
...
public static Object CreateThresholdExample() {
    return LoadStrikeThreshold.metricPredicate(stats -> stats.counters.stream().anyMatch(counter -> counter.value >= 1), 1, 2d);
}

/** Attach the threshold to the baseline GET scenario. */
public static Object AttachThresholdToScenarioExample() {
    return baselineScenario().withThresholds(LoadStrikeThreshold.metricPredicate(stats -> stats.counters.stream().anyMatch(counter -> counter.value >= 1), 1, 2d));
}
      }
