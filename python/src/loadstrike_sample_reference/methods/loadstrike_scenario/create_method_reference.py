...
class CreateMethodReference:
    @staticmethod
    def create_baseline_scenario_example() -> Any:
        """Create the default GET /orders/{id} scenario."""
        return LoadStrikeScenario.create("orders.get-by-id", execute_order_get)

    @staticmethod
    def create_scenario_and_add_simulation_example() -> Any:
        """Create the baseline scenario and immediately add a load simulation."""
        return LoadStrikeScenario.create("orders.get-by-id", execute_order_get).with_load_simulations(LoadStrikeSimulation.iterations_for_constant(1, 3))
