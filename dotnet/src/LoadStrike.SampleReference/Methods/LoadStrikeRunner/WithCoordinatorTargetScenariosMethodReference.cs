using System;
    ...
    /// <summary>Restrict coordinator-owned scenarios in distributed mode.</summary>
    public static class WithCoordinatorTargetScenariosMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Target a single scenario by name.</summary>
public static object? SelectOneScenarioExample()
{
    return LoadStrikeRunner.WithCoordinatorTargetScenarios(baseContext(), "orders.get-by-id");
}

/// <summary>Target more than one named scenario in the same call.</summary>
public static object? SelectMultipleScenariosExample()
{
    return LoadStrikeRunner.WithCoordinatorTargetScenarios(baseContext(), "orders.get-by-id", "orders.audit");
}
    }
