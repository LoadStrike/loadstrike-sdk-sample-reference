using System;
    ...
    /// <summary>Freeze the fluent runner configuration into a reusable context.</summary>
    public static class BuildContextMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Freeze the fluent runner state into a reusable context.</summary>
public static object? BuildReusableContextExample()
{
    return baseRunner().BuildContext();
}

/// <summary>Build a context after overriding the sample test name.</summary>
public static object? BuildContextAfterRenamingExample()
{
    return baseRunner().WithTestName("orders-build-context").BuildContext();
}
    }
