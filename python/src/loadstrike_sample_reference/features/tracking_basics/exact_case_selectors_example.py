...
def build():
    ...
    trackingConfig = {"tracking": _tracking_configuration(), "tracking_field_value_case_sensitive": True, "gather_by_field_value_case_sensitive": True}
    result = (LoadStrikeRunner.register_scenarios(_scenario("exact-case-selectors")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("exact-case-selectors").without_reports().run())
    return {"trackingConfig": trackingConfig, "Result": result}
