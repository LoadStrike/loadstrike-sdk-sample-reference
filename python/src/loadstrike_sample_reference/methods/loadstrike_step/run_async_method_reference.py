...
class RunAsyncMethodReference:
    @staticmethod
    def run_async_step_example() -> Any:
        """Execute the GET request through the explicit async step helper."""
        return LoadStrikeStep.run_async("get-order", create_step_context(), lambda: create_order_reply())

    @staticmethod
    def run_async_step_and_project_reply_example() -> Any:
        """Project the async step reply to the untyped reply contract."""
        return LoadStrikeStep.run_async("get-order", create_step_context(), lambda: create_order_reply()).as_reply()
