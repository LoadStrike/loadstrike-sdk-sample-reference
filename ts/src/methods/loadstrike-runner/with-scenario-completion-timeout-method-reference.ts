...
export class WithScenarioCompletionTimeoutMethodReference {
  /** Apply a shorter timeout to the runner. */
  static async ApplyShortTimeoutExample(): Promise<unknown> {
      return LoadStrikeRunner.WithScenarioCompletionTimeout(baseContext(), 5);
  }

  /** Apply a longer timeout to the runner. */
  static async ApplyLongTimeoutExample(): Promise<unknown> {
      return LoadStrikeRunner.WithScenarioCompletionTimeout(baseContext(), 15);
  }
}
