...
def build():
    # Empty Scenario Guardrails keeps the scenario setup focused on one behavior. Change the option below first when comparing suboptions.
    scenario = LoadStrikeScenario.empty(placeholders.scenario_name("empty"))
    return (LoadStrikeRunner.register_scenarios(scenario).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("empty-scenario-guardrails").without_reports().run())
