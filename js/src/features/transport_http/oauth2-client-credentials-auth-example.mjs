...
export async function build() {
  ...
  const result = await LoadStrikeRunner.registerScenarios(scenario("oauth2-client-credentials-auth")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("oauth2-client-credentials-auth").withoutReports().run();
