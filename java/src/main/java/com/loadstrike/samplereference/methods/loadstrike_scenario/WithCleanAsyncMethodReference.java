package com.loadstrike.samplereference.methods.loadstrike_scenario;
      ...
      public final class WithCleanAsyncMethodReference {
        private WithCleanAsyncMethodReference() {}
...
public static Object AttachAsyncNoOpHookExample() {
    return baselineScenario().withCleanAsync(context -> CompletableFuture.completedFuture(null));
}

/** Attach an asynchronous lifecycle hook that still registers a metric. */
public static Object AttachAsyncMetricHookExample() {
    return baselineScenario().withCleanAsync(context -> CompletableFuture.runAsync(() -> context.registerMetric(LoadStrikeMetric.counter("orders_seen", "count"))));
}
      }
