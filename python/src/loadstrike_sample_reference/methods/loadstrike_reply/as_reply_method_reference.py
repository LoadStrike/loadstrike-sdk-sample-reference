...
class AsReplyMethodReference:
    @staticmethod
    def project_typed_reply_example() -> Any:
        """Project a typed reply into the untyped reply contract."""
        return LoadStrikeResponse.ok_with_payload({"orderId": 42}, "200", 128, "ok", 3.2).as_reply()

    @staticmethod
    def project_failed_typed_reply_example() -> Any:
        """Project a failed typed reply into the untyped reply contract."""
        return LoadStrikeResponse.fail_with_payload({"error": "not-found"}, "404", 64, "missing", 3.2).as_reply()
