...
export async function build(): Promise<unknown> {
  ...
  const scenario = scenarioWithSimulation("stop-control", LoadStrikeSimulation.iterationsForConstant(1, 1));
  return await LoadStrikeRunner.registerScenarios(scenario).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("stop-scenario-control").withoutReports().run();
}
