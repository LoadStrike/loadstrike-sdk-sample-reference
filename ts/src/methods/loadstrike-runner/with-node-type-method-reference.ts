...
export class WithNodeTypeMethodReference {
  /** Set the runner to coordinator mode. */
  static async UseCoordinatorNodeExample(): Promise<unknown> {
      return LoadStrikeRunner.WithNodeType(baseContext(), "Coordinator");
  }

  /** Set the runner to agent mode. */
  static async UseAgentNodeExample(): Promise<unknown> {
      return LoadStrikeRunner.WithNodeType(baseContext(), "Agent");
  }
}
