...
export class WithLoggerConfigMethodReference {
  /** Attach a logger factory directly on the runner. */
  static async AttachInlineLoggerFactoryExample(): Promise<unknown> {
      return LoadStrikeRunner.WithLoggerConfig(baseContext(), () => buildLogger());
  }

  /** Attach the logger factory before building a reusable context. */
  static async AttachLoggerBeforeContextBuildExample(): Promise<unknown> {
      return LoadStrikeRunner.WithLoggerConfig(baseContext(), () => buildLogger()).buildContext();
  }
}
