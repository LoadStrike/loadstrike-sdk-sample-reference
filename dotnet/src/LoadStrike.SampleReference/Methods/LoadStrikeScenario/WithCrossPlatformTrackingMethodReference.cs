using System;
    ...
    /// <summary>Attach cross-system tracking configuration to a scenario.</summary>
    public static class WithCrossPlatformTrackingMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Attach the correlation configuration used for source and destination tracking.</summary>
public static object? AttachDelegateTrackingExample()
{
    return baselineScenario().WithCrossPlatformTracking(trackingConfiguration());
}

/// <summary>Start from an empty scenario and attach cross-platform tracking.</summary>
public static object? AttachTrackingToEmptyScenarioExample()
{
    return LoadStrikeScenario.Empty("orders.empty").WithCrossPlatformTracking(trackingConfiguration());
}
    }
