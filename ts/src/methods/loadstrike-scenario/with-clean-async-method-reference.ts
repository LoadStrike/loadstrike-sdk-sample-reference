...
export class WithCleanAsyncMethodReference {
  /** Attach an asynchronous lifecycle hook with no extra side effects. */
  static async AttachAsyncNoOpHookExample(): Promise<unknown> {
      return baselineScenario().withCleanAsync(async context => undefined);
  }

  /** Attach an asynchronous lifecycle hook that still registers a metric. */
  static async AttachAsyncMetricHookExample(): Promise<unknown> {
      return baselineScenario().withCleanAsync(async context => context.registerMetric(LoadStrikeMetric.counter("orders_seen", "count")));
  }
}
