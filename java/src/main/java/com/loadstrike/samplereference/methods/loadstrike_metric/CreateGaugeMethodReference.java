package com.loadstrike.samplereference.methods.loadstrike_metric;
      ...
      public final class CreateGaugeMethodReference {
        private CreateGaugeMethodReference() {}
...
public static Object CreateMetricExample() {
    return LoadStrikeMetric.gauge("orders_latency", "ms");
}
...
/** Register the metric during scenario init so the GET scenario can report it. */
        public static Object RegisterMetricDuringInitExample() {
            var metric = LoadStrikeMetric.gauge("orders_latency", "ms");
var scenario = LoadStrikeScenario.create("orders.metric", context -> executeOrderGet(context))
    .withInit(context -> { context.registerMetric(metric); return null; });
return scenario;
        }
      }
