...
export class RunMethodReference {
  /** Execute the baseline GET request inside a named step. */
  static async RunNamedStepExample(): Promise<unknown> {
      return LoadStrikeScenario.create("orders.step", async context => LoadStrikeStep.run("get-order", context, async () => createOrderReply()));
  }

  /** Project the typed step reply back into the untyped reply surface. */
  static async RunStepAndProjectReplyExample(): Promise<unknown> {
      return LoadStrikeScenario.create("orders.step", async context => LoadStrikeStep.run("get-order", context, async () => createOrderReply()));
  }
}
