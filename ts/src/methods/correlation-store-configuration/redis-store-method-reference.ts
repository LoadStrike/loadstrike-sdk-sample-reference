...
export class RedisStoreMethodReference {
  /** Call the correlation helper directly with a concrete value. */
          static async CreateCorrelationExample(): Promise<unknown> {
              const options = new loadstrike.RedisCorrelationStoreOptions();
  options.ConnectionString = "localhost:6379";
  options.Database = 1;
  options.KeyPrefix = "orders";
  options.EntryTtl = 300;
  ...
  static async UseCorrelationExampleInTrackedFlow(): Promise<unknown> {
