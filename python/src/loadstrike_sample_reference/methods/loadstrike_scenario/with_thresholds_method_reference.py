...
class WithThresholdsMethodReference:
    @staticmethod
    def attach_scenario_threshold_example() -> Any:
        """Attach one scenario-level threshold."""
        return baseline_scenario().with_thresholds(LoadStrikeThreshold.create_scenario(lambda stats: stats.all_request_count >= 1, abort_when_error_count=1))

    @staticmethod
    def attach_scenario_and_step_threshold_example() -> Any:
        """Attach more than one threshold in the same call."""
        return baseline_scenario().with_thresholds(LoadStrikeThreshold.create_scenario(lambda stats: stats.all_request_count >= 1, abort_when_error_count=1), LoadStrikeThreshold.create_step("get-order", lambda stats: stats.all_request_count >= 1, abort_when_error_count=1))
