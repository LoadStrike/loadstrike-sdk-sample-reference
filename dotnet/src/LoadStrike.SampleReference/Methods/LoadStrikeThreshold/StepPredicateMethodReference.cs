using System;
    ...
    /// <summary>Create a step predicate threshold where predicate helpers exist.</summary>
    public static class StepPredicateMethodReference
    {
...
{
    return LoadStrikeThreshold.StepPredicate("get-order", stats => stats.Ok.Request.Count >= 1, 1, TimeSpan.FromSeconds(2));
}

/// <summary>Attach the threshold to the baseline GET scenario.</summary>
public static object? AttachThresholdToScenarioExample()
{
    return baselineScenario().WithThresholds(LoadStrikeThreshold.StepPredicate("get-order", stats => stats.Ok.Request.Count >= 1, 1, TimeSpan.FromSeconds(2)));
}
    }
