...
def build():
    # Response Source Tracking is the feature being explained. The sample returns that focused object alongside a minimal run result.
    destination = _http_destination()
    result = (LoadStrikeRunner.register_scenarios(_scenario("response-source-tracking")).with_runner_key(placeholders.RUNNER_KEY).with_test_suite("orders-reference").with_test_name("response-source-tracking").without_reports().run())
    return {"destination": destination, "Result": result}
