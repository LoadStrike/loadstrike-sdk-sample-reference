...
export async function build(): Promise<unknown> {
  // Custom Logger is the main thing being demonstrated here. Keep the runner flow simple and change only this option when comparing behavior.
  const feature = { runner: runner(), loggerConfig: { provider: "dummy", minimumLevel: "Debug" } };
  const result = await feature.runner.run();
