...
export class LoadInfraConfigMethodReference {
  /** Point the runner at a temporary JSON file generated inside the sample. */
          static async UseGeneratedConfigFileExample(): Promise<unknown> {
              const paths = await writeTempConfigFiles();
  return LoadStrikeRunner.LoadInfraConfig(baseContext(), paths.infraPath);
          }

  /** Load the file and continue into a built context. */
          static async BuildContextFromGeneratedConfigExample(): Promise<unknown> {
              const paths = await writeTempConfigFiles();
  return LoadStrikeRunner.LoadInfraConfig(baseContext(), paths.infraPath).buildContext();
          }
}
