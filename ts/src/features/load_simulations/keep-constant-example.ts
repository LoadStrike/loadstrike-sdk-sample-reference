...
export async function build(): Promise<unknown> {
  // Keep Constant keeps the scenario setup focused on one behavior. Change the option below first when comparing suboptions.
  const scenario = scenarioWithSimulation("keep-constant", LoadStrikeSimulation.keepConstant(3, 20));
  return await LoadStrikeRunner.registerScenarios(scenario).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("keep-constant").withoutReports().run();
}
