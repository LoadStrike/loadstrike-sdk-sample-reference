...
def build():
    # Error Mode is the main thing being demonstrated here. Keep the runner flow simple and change only this option when comparing behavior.
    runner = _runner().with_runtime_policies(OrdersRuntimePolicy()).with_runtime_policy_error_mode("continue")
    return runner.run()
