using System;
    ...
    /// <summary>Execute a named step against the current scenario context.</summary>
    public static class RunMethodReference
    {
...
private sealed class OrdersWorkerPlugin : ILoadStrikeWorkerPlugin
{
    ...
    public Task Dispose() => Task.CompletedTask;
        ...
        /// <summary>Execute the baseline GET request inside a named step.</summary>
public static object? RunNamedStepExample()
{
    return LoadStrikeScenario.Create("orders.step", async context => { var reply = await LoadStrikeStep.Run("get-order", context, CreateTypedOrderReplyAsync); return reply.AsReply(); });
}

/// <summary>Project the typed step reply back into the untyped reply surface.</summary>
public static object? RunStepAndProjectReplyExample()
{
    return LoadStrikeScenario.Create("orders.step", async context => { var reply = await LoadStrikeStep.Run("get-order", context, CreateTypedOrderReplyAsync); return reply.AsReply(); });
}
    }
