...
export async function build() {
  ...
  const endpoints = otherBrokers().azureEventHubs;
  const result = await LoadStrikeRunner.registerScenarios(scenario("azure-event-hubs")).withRunnerKey(RUNNER_KEY).withTestSuite("orders-reference").withTestName("azure-event-hubs").withoutReports().run();
