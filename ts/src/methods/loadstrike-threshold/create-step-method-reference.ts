...
export class CreateStepMethodReference {
  /** Create the threshold from the public threshold helper. */
  static async CreateThresholdExample(): Promise<unknown> {
      return LoadStrikeThreshold.CreateStep("get-order", context => context.runStats.allOkCount >= 1, 1, 2);
  }

  /** Attach the threshold to the baseline GET scenario. */
  static async AttachThresholdToScenarioExample(): Promise<unknown> {
      return baselineScenario().withThresholds(LoadStrikeThreshold.CreateStep("get-order", context => context.runStats.allOkCount >= 1, 1, 2));
  }
}
